...
import { ChangeCharacteristics, ChapterContent_change } from "../../../questStructure.js";
...
export class Editor_Node_change extends Editor_Node
{
	...
	public render(collapsed = true)
	{
		...
		let characs: ChangeCharacteristics[] = [];
		const characsEl = Div([], [
			characsContainer,
			...
			Div("pg2-line-small", [
				CheckBox([], "Дать предметы игроку")(
					...
					inp =>
					{
						...
						addItemsEl.style.display = inp.checked ? "" : "none";
						if (!inp.checked) [addItems, this.node.addItems] = [this.node.addItems, addItems];
						text1.innerText = inp.checked ? "+Предметы " : "";
					})(),
			]),
			addItemsEl,
			Div("pg2-line-small", [
				CheckBox([], "Забрать предметы у игрока")(
					inp =>
					{
					...
					inp =>
					{
						...
						removeItemsEl.style.display = inp.checked ? "" : "none";
						if (!inp.checked) [removeItems, this.node.removeItems] = [this.node.removeItems, removeItems];
						text2.innerText = inp.checked ? "-Предметы " : "";
					})(),
			]),
			removeItemsEl,
			Div("pg2-line-small", [
				CheckBox([], "Выдать игроку достижение")(
					inp =>
					{
					...
					inp =>
					{
						...
						achievementsEl.style.display = inp.checked ? "" : "none";
						if (!inp.checked) [achievements, this.node.achievements] = [this.node.achievements, achievements];
						text3.innerText = inp.checked ? "+Достижения " : "";
					})(),
			]),
			achievementsEl,
			Div("pg2-line-small", [
				CheckBox([], "Изменить характеристики")(
					inp =>
					{
					...
					inp =>
					{
						...
						characsEl.style.display = inp.checked ? "" : "none";
						if (!inp.checked) [characs, this.node.characteristics] = [this.node.characteristics, characs];
						text4.innerText = inp.checked ? "~Характеристики " : "";
					})(),
			]),
			characsEl,
	...
	private createCharac(i: number, collapsed = true)
	{
		...
		const block = Div(["pg2-block-small", "pg2-collapsible"], [
			...
			Button("pg2-block-contextmenu", "x", () =>
			{
				...
				if (i >= 0) this.node.characteristics.splice(i, 1);
