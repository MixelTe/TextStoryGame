...
let errors = 0;
let marginLeft = 0;
export function checkData(questFolder) {
    errors = 0;
    marginLeft = 0;
    ...
    addText("");
    if (errors > 0)
        addText(`Ошибок: ${errors}`, true, true);
    else
...
function printVar(v, ifUndefined, prefix = "") {
    if (v == undefined)
        addText(`${prefix}${ifUndefined}`);
    else
        addText(`${prefix}${v}`);
}
...
function checkQuest(content) {
    const quest = parseJSON(content);
    if (quest == null)
        return;
    ...
    if (quest.hasImg)
        addText("Есть обложка");
}
function checkCharacters(content) {
    const characters = parseJSON(content);
    if (characters == null)
        return;
    ...
    characters.forEach((ch, i) => {
        ...
        if (ch.hasImg)
            addText("Есть картинки");
        addText("");
    });
}
function checkItems(content) {
    ...
    items.forEach((item, i) => {
        ...
        if (item.hasImg)
            addText("Есть картинка");
        addText("");
    });
}
function checkPlayer(content) {
...
function checkChapterPart(content) {
    const part = parseJSON(content);
    if (part == null)
        return;
...
function checkContent_splitter(content) {
    ...
    if (typeof content.conditions == "object")
        checkCondition(content.conditions, "Условие");
...
function checkCondition(cond, n) {
    addText(`${n}`);
    marginLeft++;
