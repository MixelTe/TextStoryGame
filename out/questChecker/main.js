...
window.addEventListener("drop", async (e) => {
    ...
    try {
            ...
            if (el.isFile) {
                switch (el.name) {
                    case "characters.json":
                        questFolder.characters = await readFile(el);
                        break;
                    case "items.json":
                        questFolder.items = await readFile(el);
                        break;
                    case "player.json":
                        questFolder.player = await readFile(el);
                        break;
                    case "quest.json":
                        questFolder.quest = await readFile(el);
                        break;
                    case "achievements.json":
                        questFolder.achievements = await readFile(el);
                        break;
                }
            }
        ...
        if (questFolder.items == undefined)
            throw new Error("Data is empty");
        if (questFolder.player == undefined)
            throw new Error("Data is empty");
        if (questFolder.achievements == undefined)
            throw new Error("Data is empty");
    }
    catch (e) {
...
`;
        ...
        document.body.append(Div("text-big", [], text), Div("text-medium", [], text2));
        console.error(e);
        return;
    }
