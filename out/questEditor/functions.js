...
export function removeQuest(key) {
...
export function getQuests() {
    ...
    let parsed = [];
    try {
        parsed = JSON.parse(quests) || [];
    }
    catch (e) { }
    return parsed;
}
export function getQuest(key) {
    const quests = localStorage.getItem(P(key));
...
export function saveQuest(key, quest) {
    const quests = getQuests();
    const item = quests.find(q => q.key == key);
    if (item)
        item.name = quest.quest.name;
...
export function InputPlus(classes, type, placeholder) {
    ...
    if (classes) {
        if (typeof classes == "string")
            ...
            classes.forEach(cs => input.classList.add(cs));
    }
    if (type)
        input.type = type;
    if (placeholder)
...
export function TextAreaPlus(placeholder = "", classes = []) {
    const textarea = document.createElement("textarea");
    if (classes) {
        if (typeof classes == "string")
            textarea.classList.add(classes);
        else
            classes.forEach(cs => textarea.classList.add(cs));
    }
    if (placeholder)
...
export function CheckBox(classes = [], text = "") {
    ...
    if (typeof classes == "string")
        ...
        classes.forEach(cs => checkBox.classList.add(cs));
